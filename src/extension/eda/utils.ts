...
export function withTimeout<T>(promise: T, timeout_ms: number, errorMessage = 'Operation timeout'): Promise<T> {
...
export async function getBBox(
	components: (ISCH_PrimitiveComponent | ISCH_PrimitiveComponent$1)[],
): Promise<{ minX: number; minY: number; maxX: number; maxY: number; width: number; height: number } | null> {
...
export const rmPartFromDesignator = (designator: string) => {
	...
	if (lastDotIndex !== -1) {
		...
		if (/^\d+$/.test(number)) designator = designator.substring(0, lastDotIndex);
	}
...
export function getPrimitiveById(primitiveId: string | string[]) {
	...
	else return eda.sch_PrimitiveComponent.get(primitiveId).then((r) => (Array.isArray(r) ? r[0] : r));
}
