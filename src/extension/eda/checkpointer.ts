...
const checkpointsDb = new AppDBClient(false).init('a2n_checkpoints', {
	...
	try {
		...
		if (!minor) {
			...
			if (allCheckpoints.length >= 512) {
				const sorted = allCheckpoints.sort((a: any, b: any) => a.timestamp! - b.timestamp!);
				for (const item of sorted.slice(0, sorted.length - 99)) db.checkpoints.remove({ _id: item._id });
			}
...
async function restoreCheckpoint(id?: string, allAgree = false) {
	...
	if (!id) {
		...
		const db = await checkpointsDb;
		checkpoint = (await db.checkpoints.find({ _id: id }).then((r: any[]) => r[0]).catch(() => undefined)) as Checkpoint | undefined;
	}
	...
	if (!allAgree) if (!(await confirmationMessage('Restore this checkpoint? Current changes may be lost.', 'Restore'))) return false;

	if (checkpoint === lastCheckpoint) lastCheckpoint = undefined;
...
async function readCheckpoint(id: string) {
	const db = await checkpointsDb;
	const checkpoint = (await db.checkpoints.find({ _id: id }).then((r: any[]) => r[0]).catch(() => undefined)) as Checkpoint | undefined;
	if (!checkpoint) return null;

	return { _id: checkpoint._id, timestamp: checkpoint.timestamp, pageId: checkpoint.pageId, content: checkpoint.content };
}
