...
export const placeComponent = async (
	data: { libraryUuid: string; uuid: string },
	{ x, y, rotate, mirror, addIntoBom, addIntoPcb, subPartName }: { x: number; y: number; rotate?: number; mirror?: boolean; addIntoBom?: boolean; addIntoPcb?: boolean; subPartName?: string },
) => {
	let maybeLibUuid;
	...
	for (const lib of maybeLibUuid) {
		try {
			const compPromise = eda.sch_PrimitiveComponent.create(
				{ uuid: data.uuid, libraryUuid: lib },
				to2(x),
				to2(y),
				subPartName,
				rotate,
