...
export const searchComponentInSCH = async (designator: string) => {
	...
	// @ts-ignore - fix for easy eda pro v2.2.47
	...
	const components = await getPrimitiveById(promIdComponent).catch(() => undefined);
...
export async function getAllPrimitivePins() {
	const promIdComponent = await eda.sch_PrimitiveComponent.getAllPrimitiveId().catch(() => []);
	const pins = await Promise.allSettled(
		promIdComponent.map(async (id) => ({
			primitiveId: id,
			pins: await eda.sch_PrimitiveComponent.getAllPinsByPrimitiveId(id).then((p) => p ?? []).catch(() => []),
		})),
	);
	return pins.filter((result) => result.status === 'fulfilled').map((result: any) => result.value!).filter((item) => item.pins);
}
...
export const findPin = async (
	designator: string,
	pin_: { num: number | string; name?: string },
	...
	await Promise.all(
		Object.entries(placeComponents).map(async ([pdesignator, placedComp]) => {
			pdesignator = rmPartFromDesignator(pdesignator);

			if (pdesignator === designator) {
				const c = await getPrimitiveById(placedComp.primitive_id).catch(() => undefined);
				components.push(c);
	...
	if (pinNumber == 1 && pins.length === 1 && pins[0].length === 1) {
		pin = pins[0][0];
